...
entities = ["total"]  #, "ls_growth" # Add more if needed ["total", "ls_growth"]
model_years = [2052] # 2042 , 2052

# === File naming rules based on entity ===
pop_file = lambda year, entity: (
    Path(r"T:\Yan_Kavana\tem_inputs_isaac\landuse\pop.dvec") if year == 2023 else
    Path(fr"I:\Data\D-Log\DLIT\Outputs\test21_v0.20\M4_split\dvec_pop\pop{'_large' if entity == 'ls_growth' else ''}_{year}.dvec")
)

emp_file = lambda year, entity: (
    r"T:\Yan_Kavana\tem_inputs_isaac\landuse\emp.dvec" if year == 2023 else
    fr"I:\Data\D-Log\DLIT\Outputs\test21_v0.20\M4_split\dvec_emp\emp{'_large' if entity == 'ls_growth' else ''}_{year}.dvec"
)

hh_file = lambda year, entity: (
    r"T:\Yan_Kavana\tem_inputs_isaac\landuse" if year == 2023 else
    fr"I:\Data\D-Log\DLIT\Outputs\test21_v0.20\M4_split\dvec_hh\hh{'_large' if entity == 'ls_growth' else ''}_{year}.dvec"
)
...
for entity in entities:
    ...
    hh_segmentation = ["accom_h", "ns_sec", "adults", "car_availability", "children", "adult_nssec"]
    ...
    for year in model_years:
        ...
        trip_rate_paths_hb_attr = {
            i: fr"T:\ThomasPrince\TEM I-Drive Comparison\Inputs\02-HBAttraction\trip_rates_p{i}.hdf"
            for i in range(1, 9)
        }
