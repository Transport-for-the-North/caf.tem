...
from .inputs import  AttractionModelPaths, TEMSegmentations, AttractionTripRates, TEMExportPaths
from .utils import *
...
class AttractionModel_TP:
    ...
    def __init__(
            self,
            export_paths: AttractionModelPaths,
        ...
        self.emp_landuse_path = emp_landuse_path
        self.hh_landuse_dir = hh_landuse_dir
        self.hh_landuse_prefix = hh_landuse_prefix
        
    def run(self):
        
        landuses: dict[str, cb.DVector] = {
            "emp": self.read_emp_lu(),
            "hh": self.read_hh_lu()
        }

        trip_rates: dict[int, cb.DVector] = {
            p: self.read_trip_rate(p) for p in self.trip_rates_paths
        }

        for p in trip_rates:

            lu_type = landuse_by_purpose[p]
            landuse = landuses[lu_type]
            
            trip_rate = trip_rates[p]
            pure_demand: dict[int: cb.DVector] = {}
            for year in self.export_paths.path_years:
                pure_demand[year] = self.pure_demand(trip_rate, landuse, year)

            pass

    def read_emp_lu(self):
        
        ### Align zoning
        landuse = cb.DVector.load(self.emp_landuse_path)
        landuse = cb.DVector(
            segmentation=landuse.segmentation,
            import_data=landuse.data,
            zoning_system=cb.ZoningSystem.get_zoning("lsoa_2021")
        )
        desired_zoning = cb.ZoningSystem.get_zoning(self.export_paths._zoning_system)
        landuse = landuse.translate_zoning(desired_zoning, check_totals=False, no_factors=True)
    ...
    def read_trip_rate(self, p):
        ...
        trip_rate = trip_rate.translate_zoning(cb.ZoningSystem.get_zoning(self.hb_attraction_model_paths._zoning_system),
                                               check_totals=False, 
                                               no_factors=True)

        return trip_rate

    def pure_demand(self, trip_rates: cb.DVector, landuse: cb.DVector, year: int):
        path = self.hb_attraction_model_paths.export_paths.pure_demand[year]
        attr = trip_rates * landuse
        attr.save(path)
